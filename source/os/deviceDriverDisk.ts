module TSOS {
    ...
    export class DeviceDriverDisk extends DeviceDriver {
        ...
        //Note: My OS only stores files for how big they are, even if its a program loaded in
        //This means that programs that are 256 which would usually store as 5 blocks in Disk
        //Will only store as many blocks as they need
        //For example: A9 02 8D 40 00 00 would only store in 1 block instead of 5
        //I can get away with this because we always know that a process loaded onto Memory is 256
        //So when we take a process from Disk to Memory we can put 00's in until it hits the 256 limit
        //Because our Memory is segmented evenly this is possible
        //Originally I was going to do it the 5 block way, but I did not for 2 reasons
        //Reason 1: It saves space on disk and feels more efficient space wise to me
        //Reason 2: You stated in class that at this point we can take creative liberties with our OS's, and this is one of my OS's quirks :)
                ...
                for (let j = 0; j < _Disk.blocks; j++) {
                    if (i == 0 && j == 0) {
                        ...
                        if (block[0] == "1") {
                            ...
                            let existFileName = block.substring(4);
                            let checkName = existFileName.split("00");
                            let nameArr=this.hex_to_ascii(checkName);
                           // _StdOut.putText(nameArr);

                            //This will make sure we do not list the Swap Files that should be hidden because they cannot be deleted
                            if(nameArr[0]=="*"){
                                //We do nothing
                            }
                            else{
                                //Add it to file name array
                                listofNames.push(nameArr);
                            }
                        }
                    }
                }
